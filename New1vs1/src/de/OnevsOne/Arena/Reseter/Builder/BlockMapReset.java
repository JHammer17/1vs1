...
//String[] Datas = {"" + minX, "" + minY, "" + minZ, "" + maxX, "" + maxY, "" + maxZ, "" + akX, "" + akY, "" + akZ, "" + Welt, "" + XPaste, "" + YPaste, "" + ZPaste, "" + PasteWelt, "" + ArenaID};
	...
	public BlockMapReset(Location fromPos1, Location fromPos2, Location to, main plugin, String arena, int maxBlocks) {
		...
		plugin.resetMgrArena.put(arena, this);
	}
	...
	public void copy() {
		...
		if(!plugin.protectedWordls.contains(to.getWorld().getName())) {
			plugin.protectedWordls.add(to.getWorld().getName());
		}
		...
		runnable = Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, new Runnable() {
			...
			public void run() {
				...
				int minDistanzWalls = plugin.minArenaBuildDistanceWalls+1;
				int minDistanzTop = plugin.minArenaBuildDistanceTop;
				int minDistanzBottom = plugin.minArenaBuildDistanceBottom+1;
				...
				if(readyX && readyY && readyZ) {
					...
					Bukkit.getScheduler().cancelTask(runnable);	
					 ...
					 plugin.ArenaPos1.put(arena, Ecke1);
					 plugin.ArenaPos2.put(arena, Ecke2);
					 plugin.ArenaCorner1.put(arena, to);
					 plugin.ArenaCorner2.put(arena, acLoc);
					 
					 
					 
					 plugin.getRAMMgr().saveRAM(arena, "Ready", "true");
						
					 plugin.getRAMMgr().saveRAM(arena, "Corner1.X", "" + plugin.ArenaCorner1.get(arena).getBlockX());
					 plugin.getRAMMgr().saveRAM(arena, "Corner1.Y", "" + plugin.ArenaCorner1.get(arena).getBlockY());
					 plugin.getRAMMgr().saveRAM(arena, "Corner1.Z", "" + plugin.ArenaCorner1.get(arena).getBlockZ());
						
					 plugin.getRAMMgr().saveRAM(arena, "Corner2.X", "" + plugin.ArenaCorner2.get(arena).getBlockX());
					 plugin.getRAMMgr().saveRAM(arena, "Corner2.Y","" + plugin.ArenaCorner2.get(arena).getBlockY());
					 plugin.getRAMMgr().saveRAM(arena, "Corner2.Z", "" + plugin.ArenaCorner2.get(arena).getBlockZ());
						
					 plugin.getRAMMgr().saveRAM(arena, "Pos1.X", "" + plugin.ArenaPos1.get(arena).getBlockX());
					 plugin.getRAMMgr().saveRAM(arena, "Pos1.Y", "" + plugin.ArenaPos1.get(arena).getBlockY());
					 plugin.getRAMMgr().saveRAM(arena, "Pos1.Z", "" + plugin.ArenaPos1.get(arena).getBlockZ());
						
					 plugin.getRAMMgr().saveRAM(arena, "Pos2.X", "" + plugin.ArenaPos2.get(arena).getBlockX());
					 plugin.getRAMMgr().saveRAM(arena, "Pos2.Y", "" + plugin.ArenaPos2.get(arena).getBlockY());
					 plugin.getRAMMgr().saveRAM(arena, "Pos2.Z", "" + plugin.ArenaPos2.get(arena).getBlockZ());
				
					 plugin.getRAMMgr().saveRAM(arena, "Arena.World", plugin.ArenaPos2.get(arena).getWorld().getName());
						
					 plugin.ResetingArenas.remove(arena);
					 delete();	
	...
	private void delete() {
		while(plugin.resetMgrArena.containsKey(arena)) plugin.resetMgrArena.remove(arena);
	}
}
